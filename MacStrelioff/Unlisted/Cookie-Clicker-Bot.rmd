...
The base agent class, `agent_class_naive`, contained many helper functions as well as the naive investment purchasing logic. This section explains the code used for the helper functions that extracted information from the game interface. The different strategies and their performance are covered completely in the next two sections, so this section can be skipped if you are uninterested in the helper functions.

### Browser and Game Initialization

The agent was initialized with a web driver object, created with: `driver = webdriver.Chrome('./chromedriver')`. The first part of the `__init__` method is copied below:
...
First the agent navigates the webdriver to the Cookie Clicker URL. Then it pauses for 10 seconds to let the page load. After that, the agent finds the big cookie web element and assigns it to attribute `big_cookie`. The remaining lines above initialize various attributes that are either used in purchasing logic, or used to log data used to assess agent performance. 

The second part of the `__init__` method navigated to the 'options' tab and changed the game settings. One important option to change is the `numbersButton` option -- this toggles between displaying numbers numerically, '1,000,000', versus with words, '1 million'. Since the balance is scraped from this text, the agents require that numbers be displayed numerically. The other settings were changes to maximize game performance. Code for this second part of `__init__` is shown below:
```{python,eval=FALSE}
...
A simple, but important, function is that which effects the cookie click, `click_cookie()`. The game also has golden cookies that provide a large lump sum of cookies, or increases in revenue. Golden cookies appear at random times, and at random locations on the screen. The second function below identifies and clicks these golden cookies until there are no more golden cookies on the screen, and logs the number of golden cookie clicks.
...
Upgrades are another way to spend cookies. The function below finds affordible upgrades and purchases (click) them, starting with the most expensive avilable upgrade. 
...
Balance and revenue were important for some of the agents described below, and were also logged as a metric to compare the agents. To facilitate both of these needs, I created a method that took an argument `epoch`. This method parses the text above the big cookie to convert a string like '177 cookies per second: 1.1' to extract the balance of 177 and revenue of 1.1. If the method is called without an `epoch`, then it only updates the balance and revenue for use in purchasing logic presented below. Alternatively, if an `epoch` is passed, then this method also logs the balance, revenue, and eopch or click number, so that this information can be compared across agents. The code for this is shown below:
...
One challenge was that the revenue was only viewable for owned buildings. When a new building was unlocked, but not yet purchased, the revenue was not viewable. To address this, I assumed a revenue of infinity for unowned buildings. This would incentivise agents that bought based on maximum revenue to save and purchase the new building -- which was reasonable because these buildings had higher revenue than the owned buildings. The last few lines in the code chunk above extract the revenue for owned buildings, and impute a revenue of infinity for unlocked and unowned buildings. 
...
Each agent class was instantiated as `agent` and run for 100,000 big cookie clicks, with the purchasing logic being run every 200 clicks. The Naive algorithm ran for approximately 3,550 seconds, while the other two ran for around 3,700-3,800. The differences in runtime were small enough that I wasn't worried about differences in the performance metrics below being attributable to extra income earned from a longer runtime. 
...
- best strategy might be a combination between MaxROI and MinWait
