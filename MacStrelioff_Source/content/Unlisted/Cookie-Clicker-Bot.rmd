...
The agent was initialized with a web driver object, created with: `driver = webdriver.Chrome('./chromedriver')`. The `__init__` method is shown below:
...
called without epoch, this just updated balance and revenue for purchasing logic described in the strategy sections below. Called with epoch, this would log balance and revenue for analyzing the agent's performance. 
