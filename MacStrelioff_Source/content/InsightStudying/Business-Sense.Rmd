---
title: "Product Sense"
author: "Mac Strelioff"
date: "`r Sys.time()`"
math: true
menu:
  InsightStudying:
    parent: Foundations
    weight: 99
linktitle: Product Sense
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)

# libraries
library(entropy)

# set seed for reproducability
set.seed(10201991)
```


# Question Types

1. Experimentation: "Launching new app/feature, how do you determine if it is a success?"
2. Unexpected Change: "Your manager sees a sharp, sudden drop in satisfaction survey scores, how do you investigate this?"
3. 

# Businesses and Products

- Characterize the Company and Explore its Products
- Summarize the business in 2-3 sentences
- List Offerings/Products/Features
- End Users/Customers
- Describe User Journey 
- How does the company monetize?
- Identify Metrics (high level and product specific)
- Growth/Retention
- Engagement/Transactions
- Monetization
- Brainstorm Business Problems
- Develop for specific Products and Business Areas

- Analytics can help Businesses achieve the following:
- Understand their Users/Customers
- Track health of a product or feature
- Determine if they are achieving growth
- Keep Cross-functional Teams aligned on roadmaps and goals
- Uncover Areas of Opportunity and Improvement
- Forecast future trends
- Build More Effective Products
- Reduce biased perspectives
- Avoid HIPPO (Highest Paid Person’s Opinion)

- All of your work will be done in context of specific business and product objectives
- Ensures your work is consistently aligned with goals of the company
- BI and Product Analytics Roles:
- Identify Key Metrics, develop insights and product Recommendations that will have the biggest impact
- Machine Learning driven roles:
- Develop ML models in that are in alignment with company goals, scalable and adaptive to changing needs and usage patterns
- Product Sense can inform feature engineering, segmentation, etc
- Allows you to effectively use and communicate data analysis to stakeholders and drive important business and product - decisions

- Deeply understanding the goals of a product, both for the business and the customer.
- Constantly evaluating how well the product is serving those goals, and what elements contribute or detract.
- Finding opportunities to better serve these goals.
- Understand the best set of metrics to use and deliver key insights that help drive actionable recommendations for the product or business

- A useful metric is a figure that is directly measurable and can be used to track performance over time
- Quantities
- eg. ‘Likes” per post, Daily unique visitors, pages viewed per session
- Calculated values
- eg. Revenue Per Sale, Weekly retention
- Discrepancies
- eg. Comparisons by week before, baseline averages, cohort views

- General Tips
- Ask Clarifying Questions
- Explore potential markets, customer segments, business impact
- Agree on GOAL with your interviewer
- Identify Key Metrics
- Turn goal into a measurable item(s)
- Eg. If engagement is of concern what metrics can be used to measure that?
- Daily Average number of likes per user on Facebook
- Don’t forget that Revenue is the ultimate goal for any company

- Scope potential Data Points and Variables
- What variables may help move the metric?
- How data can identify areas for simplification?
- Develop hypothetical examples (eg. specific market that may behave differently)
- Define Next Steps and Recommendations
- Describe how would you track success of your efforts
- A/B Testing
- What Success Metrics would you use?

- See Rocio's Product Analytics workshop slides for more references

# Metrics Frameworks

## AARRR

### Acquisition

#### Metrics

- Downloads
- Installs
- App Store Product Page Visits
- Site Visits
- Top-of-Mind or Aided Awareness
- People on Waiting List

#### Strategies

- SEO
- Ratings and reviews
- Paid Advertising
- Digital and Traditional Marketing
- Word of Mounth
- Promotoinal Content on free platforms (YouTube, Facebook, media outlets, ...)

### Activation

#### Metrics

- Registrations
- Session Length
- Screens Per Session
- One-Day Retention
- signups 
- downloads

CTR
Conversion rate
Activity time

- free subscribers 
- paid subscribers 
- hours of streaming content per subscriber 
- DVDs rented 

#### Strategies

- Simple download
- sign-up form
- intuitive UX
- One-Step Registration with Email/Facebook/...
- Customer On-Boarding

### Retention

#### Metrics

- n-Day Retention
- Monthly Active Users
- Session Frequency

- Retention rate
- Churn rates
- DAU/MAU 

- Conversion rate from free to paid subscription 
- Subscription renewal rate 

#### Strategies

- Minimize pain points
- Cross platform availability
- Proactive Communication
- Loyalty Bonuses
- Push Notifications
- Re-Engagement Ads

### Referral

#### Metrics

- # referral codes used 
- Referrals
- App Store Ratings and Reviews
- Social Sentiment

#### Strategies

- Rating/Review Prompts
- One-Click Sharing
- Contacts List Integration
- Incentivized Sharing

### Revenue

#### Metrics

- RPU: Average Revenue Per User 
- LTV: Customer Lifetime Value 
- App Purchases / Subscriptions
- In-App Transactions
- Ad Revenue

Acquisition cost
LTV 
Revenue
Profit
Costs

#### Strategies

- Proportion of free users
- Proportion of paying active users
- Revenue / Customer
- Sales and Promotions
- Downloadable Content
- Personalization
- Frequent Updates

## Conversion Funnel

### Discovery or Awareness

- unique visitors to the website 
- clicks on ads 
- ppl at events 

- # visitors to site, 
- # searches for that city, 
- # host listings for that city, 
- # clicks on city listings, 
- # contacts initiated (and reply rate), 
- # successful bookings

- Change/growth in any of these metrics weeks to week
- Time spent looking at listings, time spent creating new listing
- Average rating of local listing, average number of nights booked


For Netflix

- visits to site 
- clicks on ads 
- blog visits 
- followers on social media 
- video views 
- document views or downloads (white papers, ebook)

### Realization of Need or Interest

- # free accounts created 

### Consideration, Evaluation, Desire

- ppl with items in shopping cart 
- ppl who saved items to favorite list 

### Conversion or Purchase

- orders 

### Retention, Repetions

- Repeat orders


# Tools

## Funnel Analysis

- Conversion Funnel
- Engagement Funnel

## Root Cause Analysis

# Relation to Behavioral Theory

- Successive approximations describes people moving through a user funnel. Each level should be the reward for the previous level.

# YouTube Example

The YouTube recommendation algorithm favors videos with high watchtime. 

[video](https://creatoracademy.youtube.com/page/lesson/discovery?cid=get-discovered&hl=en) on the algorithm.

- titles
- thumbnails
- descriptions
- how other viewers seem to enjoy it
- likes/dislikes
- comments
- view duration

[series](https://www.youtube.com/watch?v=gTrLniP5tSQ&list=PLpjK416fmKwQK6_REczCaBQ1x1jyHvZAF) on how search and discovery works. 

Search: 

- videos ranked match between query and title, description
- tip: can make videos on trending content. 
- tip: can use google to find common search terms.
- engagement and watch time

Suggested videos: 

- can be from any channel
- suggest viewers watch another video
- playlists, links, cards, boxes, screens to suggest your other videos

Home screen:

- performance of video
- personalization, video's match for a viewer's previous videos watched, and which videos they've already seen

Trending:

- specific to viewer's country
- combines popularity with novelty
- considers rate of growth in views
- includes creater on the rise
- favors sharable videos

Subscribers: 

- biggest fans
- subscriptions tab shows videos from channels that a viewer is subscribed to
- subscription content also appears in home screen
- can encourage subscriptions by asking users to subscribe, and convaying the value to them

Notifications:

- sent to subscribers when a new upload is made public
- stochastic, so it helps to jumpstart viewership but isn't the best in the long run


# Problem Types and Strategies

## New Product

Overall product sense is about what to build and why. 

1. Overarching goals / stage of the company
2. Contextualize: Draw a user funnel or user flow for the specific product
3. Draw a dashboard, then populate with platform and product metrics

# Overall resources

- [https://www.youtube.com/playlist?list=PLOhHNjZItNnM5r2JCX9KzrFH5WYWKrpAA](https://www.youtube.com/playlist?list=PLOhHNjZItNnM5r2JCX9KzrFH5WYWKrpAA)

# Deep-Dives

[Deep Dive guide](https://sites.google.com/insightdatascience.com/interviewstrategies/interviews/interview-logistics/deep-dives?authuser=0)


# Frameworks

## AARRR

### Acquire

- [Growth Case Studies](https://sujanpatel.com/marketing/growth-levers/)
- 

### Activate

### Retention

- Churn


### Referral

### Revenue


## User Funnel





# Platform Concepts and Metrics

## Users


### User Flow

- Traffic source: Where users come from (search, referral, ... )
- number new visits, percent new visits
- Page visits: users arriving at page
- Bounce rate: the percentage of visitors to a particular website who navigate away from the site after viewing only one page.
- Pages viewed / visit
- Time on pages
- Time on site
- Conversion (purchase, reservation, end-goal action)

### Intent

### Lifetime Value

## KPIs

[Engagement Metrics](https://mixpanel.com/topics/important-user-engagement-metrics-apps/)

Active user metrics are especially important for monetization with ad revenue. 

Daily active users: 
$$
DAU = n_{users,day}
$$

Average daily active users:
$$
\begin{aligned}
ADAU &= \frac{\text{total number of active users in a month}}{\text{days in the average month}}\\
&= \frac{\sum_{days}^{n_{days}} DAU_{day}}{n_{days}}
\end{aligned}
$$

Monthly active users: 
$$
\begin{aligned}
MAU &= \frac{\text{total number of active users in a year}}{12} \\
&= \frac{\sum_{days}^{365} DAU_{day}}{n_{months}}
\end{aligned}
$$

Stickiness: Here the numerator is a measure of activity within a month, and the denominator is a measure of activity within a year. 
$$
\begin{aligned}
S &= \frac{ADAU}{MAU} \\ 
  &= \frac{\frac{1}{n_{days}}\sum_{days}^{n_{days}} DAU_{day}}{\frac{1}{n_{months}}\sum_{days}^{365}DAU_{day}}\\
  &= \frac{n_{months}\sum_{days}^{n_{days}}DAU_{day}}{n_{days}\sum_{days}^{365}DAU_{day}}
\end{aligned}
$$

[alternatively](https://blog.popcornmetrics.com/5-user-engagement-metrics-for-growth/), stickiness could be measured as $DAU$ over $MAU$

- Daily active users: number of users in a day
- Active users: Average Daily Active Users = monthly users / days in average month
- Active users: Monthly Active Users = users in a year / months in year
- Stickiness: Average Daily Active Users / Monthly Active Users = 

- Support ticiets: new tickets + unresolved tickets
- Time to ticket resolution

- [https://www.klipfolio.com/resources/kpi-examples](https://www.klipfolio.com/resources/kpi-examples)

## Monetization 

- customer lifetime value (LTV)
- monthly recurring revenue (MRR)

# Product Concepts and Metrics

## Demand

## Engagement

- clicks
- bounce rate on subsequent page -- used combined with clicks to mitigate click bait
- Shares
- Upvotes
- DAU
- time of sessions

## Value

- Shares
- Upvotes

## Add from meeting with Alejandro

- User data (ways users use the app)
- Usage data (ways the app interacts with devices, networks, servers, product versions, ...)


# OLD NOTES: 

# Insight topics list:
engagement, search engine optimization (SEO), key-performance indicators (KPIs),

Experiment design, A/A and A/B testing, power analysis; 

ML focused business case study (features, algorithms, validation, value), business focused data challenges



# Template

## Value of a new feature

e.g. higher engagement, better labels for training data or other user behaivors

## Deamand for a feature

- Surveys
- Focus Groups
- UX interviews / shadows
- Retrospective analysis (looking at past user behavior)

## Treatment Design

How exactly will this be implemented in the treatment group? 

Specify treatment and control groups.

### Issues

- Networks: Users that interact might be in different conditions
- Learning effects: Users might respond differently initially than they do asymptotically. Can run experiments longer until the treatment effect seems to stabalize. 

## Constructs and Metrics

Engagement:

- Lift
- CTR
- Duration

Monetization: 

- CTR
- revenue per user
- add revenue
- ...

Guard Rail Metrics:

- 

## Metric evaluation and selection

1. What if we optimize for ... ?: The choice of metrics determines what the platform is being optimized for, so think about what will happen if the platform is optimized for the chosen metric.

### Method

### Size 

Power analysis

### Duration

Bandits versus A/B tests, 

### Other considerations: 

- Heterogenous treatment effects


# Example Question Formats


# Mock questions and templates 

## Monitoring Twitter serach

Metrics:

Measure health: 

Working well + being used … 

Track number of users using search! 

Consider people who have used the search once.. (can assess need) 


Use of search

 # users that use search

Usage (prop of users that interact with search)

 # users using search / # active users

Proportion of times a user comes and uses search 

Time interval(s):

Daily

Weekly

 # number of search sessions 

Total search volume (server traffic, … -- important!) 


Quality of search


Chose any suggestion

Rank of suggestion that they chose

Look at total number of searches, and searches in different categories / segmentations

Depends on infrastructure / ease of cutting later


-- favorites, retweets, … Other metrics from a deep-dive.


CUTS IN  DATA


Think of groups that might be using feature differently

New vs old users

Device type (iphone vs android) 

Browser



Conditions where the feature may be behaving differently (languages, countries, …) .. 

Note: countries would have different bandwidths (country easier to measure than bandiwdth itself) 

Language that the search is using. 




SPIKE IN THE METRIC # of searches


Bug


Event



Think of more people on platform moving with searches… 


 # clicks and # searches -- if it’s a bug, then clicks prob wouldn’t also spike.

Impressions, clicks, … 


Top of funnel, 


Logging in, going to search, writing stuff, hitting search .


‘Ok lets write down some ideas and see how we feel about them … ‘


Looking for intuition about how things will move and interact? 


Looking for: 

Organized in thinking

Incl understanding of problem space

Communicating thinking well







[Giulio Palombo's collection of business focused data challenges](https://datamasked.com/)

Company deep dives; data+schema brainstorming; ML algorithms; Insights;

Company blogs are a great resource: check out how StitchFix, Instacart use data science.

More company deep dives!




























