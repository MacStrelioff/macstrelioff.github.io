...
If $a$ or $b$ have different signs and $Cov(X,Y)>0$, then the variance between is reduced because the covariance is subtracted off. This is a justification for within-participant designs, where $X$ represents the first measurement, $Y$ represents a future measurement, $a=-1$ and $b=1$ to reflect a difference comparing the second and first measurements ($Y-X$), and since the measurements come from the same person here, it is reasonable to assume they are correlated ($Cov(X,Y)>0$). In this case, $Var(X,Y)=Var(X)+Var(Y)-Cov(X,Y)$, which is not larger than the variance that would be obtained if the measurements were from independent samples. 

A crucial concept in machine learning is the [Bias-Variance tradeoff](https://en.wikipedia.org/wiki/Bias%E2%80%93variance_tradeoff). The Bias-Variance tradeoff is a decomposition of the expected prediction error when estimating a function $g(x)$ with a model $\hat{g}(x)$; $g(x)-\hat{g}(x)$. To see it's relation to variance, first rearrange the variance equation above:
...
Then defined the random variable $X$ to be the errors of the model; 

$$
\begin{aligned}
E((g(x)-\hat{g}(x))^2) &= Var((g(x)-\hat{g}(x))^2)+E((g(x)-\hat{g}(x)))^2 \\
&= (g(x)-E(\hat{g}(x)))^2 + (E(E(\hat{g}(x))-\hat{g}(x))) + Var(g(x))\\
...
This arises in hierarchical models. 
