...
General form: 
```{,eval=FALSE,echo=TRUE}
WINDOW_FUNCTION() OVER(PARTITION BY [grouping row] ORDER BY [ordering row] [DESC|ASC]) 
```
...
- `exists` to get max mid from movie
```{,eval=FALSE,echo=TRUE}
select m1.mid 
from movie as m1
where not exists (select * from movie m2 where m2.mid > m1.mid and m2.mid <> m1.mid)
```
...
- casting: ` CAST(25.65 AS int)`, also see `CONVERT()`
- rounding: `trim(to_char(sum(b.slots)/2.0, '9999999999999999D99')) as "Total Hours"`
- String replacement: `translate([string column], '[chars to replace]', '[replacement char]')` 
- String replacement: `regexp_replace(telephone, '[^0-9]', '', 'g')`
- Strip chars: `substr ([string],1,1)`
- Strip chars: `LEFT([string],[n to strip])`
- Strip chars: `RIGHT([string],[n to strip])`
- Strip chars: `SUBSTRING([string],[start idx],[end idx])`
- Pad chars:  `lpad(cast(zipcode as char(5)),5,'0')`

### Dates

- Extract from timestamp: `extract([month] from [timestamp])`
- Truncate timestamp: `date_trunc(['year'],[timestamp])`

### Other

- `exists ([subquery])`: True if the subquery is not empty
- `ROLLUP(facid,month)`
...
select re.name
from reviewer re
where re.rid in (select rid from rating where rating.ratingDate is NULL)
```
...
```{,eval=FALSE,echo=TRUE}
select avg(case when year < 1980 then mavg end) - avg(case when year > 1980 then mavg end)
from (
select avg(r.stars) as mavg, year
from rating as r 
join movie as m 
on r.mid=m.mid 
group by m.title
...
/* Using having from Gina*/
SELECT mov.title, AVG(rat.stars) as mov_avg_rating
FROM rating rat
INNER JOIN movie mov ON mov.mID = rat.mID
GROUP BY rat.mID
HAVING mov_avg_rating = (SELECT MIN(sub1.avg_rating) as lowest_rating
    FROM (SELECT AVG(stars) as avg_rating FROM Rating GROUP BY mID) sub1) 
```
...
7. Classify facilities into equally sized groups of high, average, and low based on their revenue. Order by classification and facility name.
```{,eval=FALSE,echo=TRUE}
...
8. For each day in August 2012, calculate a rolling average of total revenue over the previous 15 days. Output should contain date and revenue columns, sorted by the date. Remember to account for the possibility of a day having zero revenue. This one's a bit tough, so don't be afraid to check out the hint!
```{,eval=FALSE,echo=TRUE}
select 	dategen.date,
	(
