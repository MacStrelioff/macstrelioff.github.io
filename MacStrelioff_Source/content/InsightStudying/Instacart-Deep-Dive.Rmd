...
Interviewers: [Gregory Renner](https://www.linkedin.com/in/gregxrenner/), [Ben Knight](https://www.linkedin.com/in/benjamin-knight/)

- stats interview for around 15min

- coding challenge on my own. should have email with link. 

- shouldn't need full hour.. 
...
- Project similar to my Insight project
- Extensive SQL and stats training
- Previously in navy

- logistics team works to make sure shopper app is useful to the shopper

<!-- Questions -->

- first Q giving answer to two audiences; 
- basic/intermediate stats
- audiance more lay audience

- what is a p value

- working on analysis
- asked about analysis
- they express concern at outliers could invalidate results

copy query and output into answer field then submit

select avg(case when rating = 5 then basket_size end) as "Avg basket size for 5 stars",
       avg(case when rating < 5 then basket_size end) as "Avg basket size for <5 stars",
       avg(case when rating is NULL then basket_size end) as "Avg basket size for NULL rating"
from (select o.order_id as id, 
       r.ratings as rating,
       o.basket_size as basket_size
from orders as o
left join ratings as r 
on r.order_id = o.order_id) as t1

- Stats tech stack? What methods do you use often? 

# Onsite

great article on [experimentation and metrics](https://tech.instacart.com/it-all-depends-4bb7b22e854b)

### Notes

- hiring in customer, performance (incentive structures, ...), and ads

<!-- Questions -->

- prep for the project? 
- pandas, SQL to pull data, stats in Python, other packages, ... ? 

- in practice, how are funnels constructed

- Salary talk with Charlene? She said she'd bring it up at the onsite. 

- feedback from phone screens
- Brett liked framework, clarity of thought, how I broke problem down. Could tell I have solid background in experimental / causal inference. 
- Improvement: 
- - clear I didn't have product experience, lacked vocabulary to talk about problem 
- talking about retention, churn without using those terms / tieing it to a problem solving framework. 
- today / tomorrow, try to read blogs/articles on (not giving much away) - look for product management interview prep materials. Try to find articles on growth mindset, acquisition strategies, retention, things related to how we make a product grow / be profitable. 

- If I have app downloaded, think about user experience, painpoints, with mindset of how the company should think about it, what's important to measure, observe, ... . 
- startup growth blogs
- tradeoffs between acquisition, retention, ROI, cost per acquisition, incremental gains and costs, ...

- second thing: clear experimentation was my strong suit, might have gone into it a little too fast. 
- didn't articulate problem, break it down, ... .
- simple things first (segmentation, funnel, .. )
- breaking down metrics
- want to make sure to identify root cause in question before designing an experiment.
- this prevents doing a bunch of experiments that go nowhere
- MECE: a consulting guideline for a framework. Frameworks should be composed of small parts that are mutually exclusive and collectively exhaustive. 
- Break problem down into little hypotheses.

- stats focused on experimentation
- problem solving are focues on product

- project, think of it as a condensed data challenge
- should be able to use any or all of those tools (SQL, Python, R)
- again think about framework and execution time
- make sure data manipulation skills are on point
- not like a ML exercise
- trying to get signal on day to day skills I'll need

- roles more focused on product analytics
- dashboards, metrics
- product focused role, heavily on experimentation and CI

- bottom line drives what you work on, ... 
- fulfillment side, focusing on how to properly staff the demand. 
- enought people to work? 
- the people working have work available? 
- best people get work more easily? 

- customer/growth
- how can we get people to place first, second ,... orders?

- nobody on interview list on ads team.
- ads: make sure we show right ads to right people and getting most profit out of that exposure.

- p's : priority
- SLA: service level agreements.
- top priority: increase number of new activations

- do food items on signup screen increase clickthrough or conversion? 
- can we predict retention or churn? 

- Most projects had a monetary amount in the results section
...
- Instacart conversion funnel

- Instacart repeat user patterns
- what would grow repeat users? 
- what would grop user acquisition? 

for some metrics and models in production at instacart, see [this video](https://www.youtube.com/watch?v=7PBM_sAvMPA) at 5:01.

Good article on [AARRR](https://medium.com/@ms.mbalke/aarrr-framework-metrics-that-let-your-startup-sound-like-a-pirate-ship-e91d4082994b) framework.
...
- customer/growth
- how can we get people to place first, second ,... orders?
- - What about a lessening discount on the first 3 orders -- 20% off, 10% off, 5% off, full price ... .
- - Referral bonuses
- - Randomized referral bonouses to incentivize multiple referrals

- Retention:
- - instacart loyalty rewards
...
- top priority: increase number of new activations
- Cues on signup page
- Dark background, padlock image on pages that require sensitive info (credit card).

## My unique value ads

Preference for high divergance outcome distributions:

- Project on choosing rooms with gambles with high divergance. 
- At Instacart, this would be like a higher probability of picking stores with more variety in their items. In an extreme case, imagina a bananna store with all different kinds of banannas, versus a typical Safeway which has some banannas along with lots of other kinds of items too.
- At Instacart, it could also be shoppers chosing to log in 

Paradox of choice and decision fatigue, and statistics people use to simplify decisions;

- Customers and multiattribute decision making;
- Decision fatigue and heuristics: Lots of stores, lots of similar products within stores
- Come up with a way of presenting the items that is consistent with the heuristics people might be using to make simple decisions when there are many options. (show price information, quantity information, other helpful statistics -- maybe neutritional information?)
- Maximizing search efficiency: In the search area, could cluster items by similarity -- stack 'bananna' cards, ... .
- Maximizing entropy in item types: In the discovery area, could organize items by dissimilarity so that a person doesn't need to scoll past 3 banannas 2 avacados, ... . Instead they see a variety of items throughout their scrolling. 
- Some users might care a lot about neutritional informaiton, so making that accessible on demand could be a huge value add of instacart versus other options, even including actually going to the store (since neutrition info isn't accessible on some in store items). 

Cues and impulsivity: 

- Chose between smaller, sooner amounts and larger, later amounts
- Associated a cue with money
- Hypothesized that the association with money would increase the 'incentive salience' of money which would induce impulsive behavior -- i.e. invrease the value of money. 
- At a summer internship the company found that having a dark background and an image of a padlocklock reduced dropoff on a page that asked for credit card information. 
- At Instacart, this would be like an image of food on the signup page inducing a higher proportion of homepage conversions. 

- cues as something people use to simplify decisions

- Shoppers and deciding when to work: Studies with taxi drivers shows that they work until they make a certain amount of money, rather than working more during peak hours and less during times of low demand. How would this play out in Instacart markets?

- Decoy effects and subscription plans; 
- features: price and number of free deliveries
- having a similar but inferior option can shift users towards the superior similar option

- Personalization -- what kind of food will most entice this person? 
- Time of day, source to page? 

- Showing an image of a juicy burger to a vegetarian, or a bag of chips to someone on a diet, might result in a lost conversion. 
...
- Scraping data from predictit
- Writing code to trade / text us when there's opportunities
- Sportsbook -- using a 'free bet' to get free money. 

### Salary

- Range sounded good.
- Depends on feedback from the interview and timing of pipelines with other companies.
- I like Instacart a lot, I like the kinds of problems, I like the people, I like the kind of data, ...
- So if I was offered something in the Sr. DS range now, I'd probably pull out of other interview pipelines and accept.

- Are salaries dependent on the team?
- Bonuses?
- Structure of raises and promotions?
- How long do employees typically spend in each level (DS, Sr., Staff)?
- Is more equity offered after 4 years?
- Other components of the offer?
- When is Instacart aiming to IPO?

### About past work

1. Insight Project
- Context: Startup focused on workspace collaboration, combined features of Google Docs with features of Github

- Problem: Only 20\% of users converted from a free trial to a subscription, client wanted to predict who would convert as early as possible

- Data: daily counts of engagements with a product feature from an account

- Features: cumulative sums, divided by number of users in the account -- a measure of how much an account had engaged with a product feature since the account's creation. 

- Approach: Classification problem, 
- SMOTE to address class imbalance (around 20\% didn't convert)
- F1 because of class imbalance and no particular preference for precision or recall..
- In hindsight, recall might have been more important, 
- because part of the use case for a model like this was to reach out to subscribers earlier on with different subscription tiers
- so I might have used an $F\beta$ score to upweight recall.
- so I think in future projects I'd spend time linking the outcomes to some concrete value. 
- Also a little more time building an understanding of the users through something like a funnel analysis.

- Result: 
- Overall I achieved decent F1 score (don't remember number)
- And my client was really happy. 
- The collaboration was really nice too -- multiple teams were engaged in meetings and appreciated what I could deliver in two weeks.

[conversion funnel](https://www.crazyegg.com/blog/ecommerce-conversion-funnel/)

- User flow
- Funnel, how many accounts have at least one user that took up each of more complex actions
- Persistant use: how many users days in a row a user logs in
- proportion of weekdays logged in 

2. Strategy Inference Project:
- Context: Participants chose between two gambling machines across 30-50 trials. 
- Project: I specified four models to predict which machine they'd choose on the next trial.
...
- feedback:
- good energy, collaboration
- work on: 
- experience, ability to ex..?
- structured thinking
- academic experience into real world peoblem
- lot of hand holding to come up with a metric or solution, were hoping for more independence.. 
- sandeep, didn't have right role currently.. and didn't have a role where I'd be able to mentor the skillsets I'd need
- if I'm able to improve in aspects of execution, structured thinking, ... 
- could reinterview on a future date after I have some experience on a team that can better mentor.
